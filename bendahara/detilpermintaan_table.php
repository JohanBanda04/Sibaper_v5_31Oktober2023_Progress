...
//        $mail->SetFrom("senderforemail340@gmail.com", "SPermohonan Konfirmasi Barang dari Pengelola"); //set email pengirim
//        $mail->Subject = "Permohonan Konfirmasi Barang dari Pengelola"; //subyek email
//        $mail->AddAddress($email_kasub_pengguna);  // email tujuan
//        $mail->MsgHTML("Halo, ".$nama_lengkap_kasub_pengguna."Terdapat Permohonan Konfirmasi Barang dari Pengelola dari ".$_SESSION['nama_lengkap'].", pada tanggal ".tanggal_indo($tgl_sekarang).",  segera cek aplikasi SIBAPER melalui akun Anda"); //pesan
//        $mail->Send();
        ...
        while($data_email_pemohon = mysqli_fetch_array($queries)){
            ...
            $query_operator_pemberi = mysqli_query($koneksi,"select * from user where id_user=$_SESSION[user_id]");
            $result_get_operator = mysqli_fetch_assoc($query_operator_pemberi);

            $get_subbidang_operator_pemberi = mysqli_query($koneksi,"select * from subbidang 
where id_subbidang='$result_get_operator[subbidang_id]'");
            $result_subbidang_operator_pemberi = mysqli_fetch_assoc($get_subbidang_operator_pemberi);

            $get_subbidang_pemohon = mysqli_query($koneksi,"select * from subbidang 
where id_subbidang='$pemohon_data[subbidang_id]'");
            $result_subbidang_pemohon = mysqli_fetch_assoc($get_subbidang_pemohon);
            ...
            $mail->Username = "senderforemail340@gmail.com"; //user email
            $mail->Password = "mtklbsimtazfowfi"; //password email
            $mail->SetFrom("senderforemail340@gmail.com", "Pemberian Barang Ke ".$pemohon_data['jabatan_detail']." ".$result_subbidang_pemohon['nama_subbidang']); //set email pengirim
            $mail->Subject = "Pemberian Barang Ke ". $pemohon_data['jabatan_detail']." ".$result_subbidang_pemohon['nama_subbidang']; //subyek email
            $mail->AddAddress($email_pemohon);  // email tujuan
            $mail->MsgHTML("Halo, ".$pemohon_data['jabatan_detail']." ".$result_subbidang_pemohon['nama_subbidang']." Akan Dilakukan Pemberian Barang oleh : "
                .$result_get_operator['jabatan']." ".$result_subbidang_operator_pemberi['nama_subbidang']
        ...
        while($data_email_pemohon = mysqli_fetch_array($query_get_email_penggunas)){
            ...
            $mail->MsgHTML("Halo, ".$dt_nama_lengkap." Terdapat Konfirmasi Penerimaan Barang Selesai dari Pengelola : ".$_SESSION['nama_lengkap'].", pada tanggal ".tanggal_indo($tgl_sekarang).",  segera cek aplikasi SIBAPER melalui akun Anda"); //pesan
            $mail->Send();
        }
    } else {
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="text-center">
                        ...
                        ?>
                        Konfirmasis Permintaan <b><?php echo $nama_lengkap_pemohon; ?></b></h3><br>
                    <center>
                                                ...
                                                if ($row['status_acc'] == 'Setuju Kasub Bendahara') {
                                                    ...
                                                    <form method="post" action="penyerahan_barang_pengguna_table.php">
                                                        ...
                                                        <input onclick="return confirm('Penyerahan Barang Ke Pengguna?')"
                                                               ...
                                                               value="xPenyerahan Barang Ke Pengguna">
                                                ...
                                                <?php } else if ($row['status_acc'] == "Tidak Setuju Kasub Bendahara") { ?>
                                                    ...
                                                    <form action="hapus_yg_tidak_disetujui.php" method="post">
                                                        ...
                                                        <button class="btn btn-danger"
                                                                title="Hapuses" data-placement='top'
                                                                data-toggle='tooltip'
                                                                type="submit"
                                                    ...
                                                    <form hidden action="edit_yg_tidak_disetujui.php" method="post">
