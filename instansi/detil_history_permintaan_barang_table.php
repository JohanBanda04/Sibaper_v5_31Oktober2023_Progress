...
('Permintaan Baru','Pengajuan Kasub')");
    ...
    $query_pengecekan_v2 = mysqli_query($koneksi, "select sementara.user_id, sementara.unit, sementara.user_id,
user.subbidang_id, sementara.bendahara,sementara.bendahara_id,sementara.id_sementara
,stokbarang.nama_brg,stokbarang.satuan,sementara.jumlah,sementara.note_kasub_pengguna,sementara.note_bendahara,
sementara.status_acc
from ((sementara inner join stokbarang 
on sementara.kode_brg=stokbarang.kode_brg) inner join user on user.id_user=sementara.bendahara_id) where unit='$unit' 
and user_id='$user_id' and tgl_permintaan='$tgl_permintaan'  and status_acc not in
...
('Permintaan Baru','Pengajuan Kasub','Tidak Setuju Kasub Bendahara'))");
    ...
    if ($jml_akumulasi == $jml_request) {
        ...
        while ($val = mysqli_fetch_array($query_pengecekan_v2)) {
            ...
            $query_data_pemohon = mysqli_query($koneksi,"select * from user where id_user=$val[user_id]");

            $data_pengelola = mysqli_fetch_assoc($query_data_pengelola);
            $data_pemohon = mysqli_fetch_assoc($query_data_pemohon);

            $email_pengelola = $data_pengelola['email'];
            $email_pemohon = $data_pemohon['email'];

            $nama_lengkap_pengelola = $data_pengelola['nama_lengkap'];
            $nama_lengkap_pemohon = $data_pemohon['nama_lengkap'];

            $jabatan_pengelola = $data_pengelola['jabatan'];
            $jabatan_pemohon = $data_pemohon['jabatan_detail'];

            $subbidang_id_pengelola = $data_pengelola['subbidang_id'];
            $subbidang_id_pemohon = $data_pemohon['subbidang_id'];



            $get_tb_subbidang_pengelola = mysqli_query($koneksi,"select * from subbidang 
where id_subbidang='$subbidang_id_pengelola'");
            $get_tb_subbidang_pemohon = mysqli_query($koneksi,"select * from subbidang 
where id_subbidang='$subbidang_id_pemohon'");

            $result_tb_subbidang_pengelola = mysqli_fetch_assoc($get_tb_subbidang_pengelola);
            $result_tb_subbidang_pemohon = mysqli_fetch_assoc($get_tb_subbidang_pemohon);

            $nama_bidang_pengelola = $result_tb_subbidang_pengelola['nama_subbidang'];
            $nama_bidang_pemohon = $result_tb_subbidang_pemohon['nama_subbidang'];
            ...
            $mail->SetFrom("senderforemail340@gmail.com", "Penerimaan Barang Oleh $jabatan_pemohon ".$nama_bidang_pemohon); //set email pengirim
            $mail->Subject = "Penerimaan Barang Oleh $jabatan_pemohon ".$nama_bidang_pemohon; //subyek email
            $mail->AddAddress($email_pengelola);  // email tujuan
            $mail->MsgHTML("Halo, $nama_lengkap_pengelola " . $jabatan_pengelola." " .$nama_bidang_pengelola
                . ", TTerdapat Konfirmasi Penerimaan Barang Oleh Pemohon Barang : "
                . $jabatan_pemohon ." ".$nama_bidang_pemohon. ", pada tanggal " . tanggal_indo($tgl_sekarang) . ",  segera cek aplikasi SIBAPER melalui akun Anda"); //pesan
            $mail->Send();
        ...
        while($dt_pengelolas = mysqli_fetch_array($query_pengelolas)){
            ...
            $mail->MsgHTML("Halo, " . $nama_lengkap_pengelolas . ", sTerdapat Konfirmasi Penerimaan Barang Oleh Pemohon Barang dari Pemohon Barang : " . $_SESSION['nama_lengkap'] . ", pada tanggal " . tanggal_indo($tgl_sekarang) . ",  segera cek aplikasi SIBAPER melalui akun Anda"); //pesan
            $mail->Send();
        }
